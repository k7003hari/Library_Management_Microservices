 ...
 * 
 * import com.ctc.exception.FineAlreadyPaidException; import
 * com.ctc.exception.FineNotFoundException; import com.cts.dto.FineDTO; import
 * com.cts.model.Fine; import com.cts.repository.FineRepository; import
 * com.cts.service.FineServiceImpl;
 * 
 * @SpringBootTest class FineServiceImplTest {
 * 
 * @InjectMocks private FineServiceImpl fineService;
 * 
 * @Mock private FineRepository fineRepository;
 * 
 * private AutoCloseable closeable; private Fine fine;
 * 
 ...
 * 
 * @Test void testIssueFine() {
 * when(fineRepository.save(any(Fine.class))).thenReturn(fine); FineDTO result =
 * fineService.issueFine(new FineDTO(null, 101L, 201L, 50.0, false,
 * LocalDate.now())); assertNotNull(result); assertEquals(101L,
 * result.getMemberId()); }
 * 
 * @Test void testPayFineSuccess() { fine.setPaid(false);
 * when(fineRepository.findById(1L)).thenReturn(Optional.of(fine));
 * when(fineRepository.save(any(Fine.class))).thenReturn(fine);
 * 
 * FineDTO result = fineService.payFine(1L); assertTrue(result.isPaid()); }
 * 
 * @Test void testPayFineAlreadyPaid() { fine.setPaid(true);
 * when(fineRepository.findById(1L)).thenReturn(Optional.of(fine));
 * assertThrows(FineAlreadyPaidException.class, () -> fineService.payFine(1L));
 * }
 * 
 * @Test void testGetFineByIdNotFound() {
